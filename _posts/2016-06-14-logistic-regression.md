...
这样的话，我们的 **logistic回归方程** 就是 y_{i}=\frac{1}{1+e^{-(x_{0}+x_{1}+...+x_{n})}} 。

有了回归方程后就可以用 **极大似然估计进行参数估计** 。假设有n个独立的训练样本，$(x_{1},y_{1}),(x_{2},y_{2})...(x_{n},y_{n})$ , $y\in (0,1)$ 。设 $p_{i}=P(y_{i}=1|x_{i})$ 是 $y_{i}=1$ 的概率，那么 $y_{i}=0$ 的概率就是 $1-p_{i}$ 。任意一个观测值 $(x_{i},y_{i})$ 出现的概率是 $P(x_{i},y_{i})=p_{i}^{y_{i}}(1-p_{i})^{1-y_{i}}}$ 。容易构造 **似然函数** $L(w)=\prod_{i=1}^{m}P(x_{i},y_{i})$ 。取对数即得到 **对数似然函数** $lnL(w)=\sum_{i=1}^{m}[y_{i}lnp_{i}+(1-y_{i})ln(1-p_{i})]$
